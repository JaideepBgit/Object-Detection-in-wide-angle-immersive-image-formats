...
class Data(data.Dataset):
    ...
    def __init__(self,li,size=(300,300)):
        ...
        for i in li:
            self.img.append(i[0])
            self.lab.append(int(i[1]-1)) # no label must be < num_classes
    def __getitem__(self, index):
        img_path = '/media/david/OS/Users/David/Documents/01 Career/01 IPCV/02 UAM/Tutored Research and Development Project/VOCdevkit/VOC2012/JPEGImages/'+self.img[index]+'.jpg'
        image = cv2.imread(img_path)
        image = cv2.resize(image,self.size)
        image = image.astype('float32')/255.0 # convert to float and normalize
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', default='CUSTOM',
                        choices=['OmniMNIST', 'OmniFashionMNIST','CUSTOM'],
