...
def convert_annotation(year, image_id, path):
    ...
    out_file = open(path + '/VOCdevkit/VOC%s/labels/difficult_%s.txt'%(year, image_id), 'w')
...
if __name__ == '__main__':
    ...
    for year, image_set in sets:
        ...
        list_file = open(path + '/difficult_%s_%s.txt'%(year, image_set), 'w')
        for image_id in image_ids:
            list_file.write(path + '/VOCdevkit/VOC%s/JPEGImages/difficult_%s.jpg\n'%(year, image_id))
